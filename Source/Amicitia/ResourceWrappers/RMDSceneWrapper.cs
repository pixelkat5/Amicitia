...
namespace Amicitia.ResourceWrappers
{
    ...
    public class RmdSceneWrapper : RwNodeWrapperBase<RmdScene>
    {
        ...
        /// <summary>
        /// Exports each clump in this scene as a .dae model, mirroring the Clumps "Export All" action,
        /// instead of dumping the raw .rmd/.rws.
        /// </summary>
        public override bool ExportToFolder(string folderPath)
        {
            if (ClumpsWrapper == null || ClumpsWrapper.Count == 0)
                return false;

            var baseName = GetExportFileName(SupportedFileManager.GetSupportedFileInfo(SupportedFileType.RmdScene));
            baseName = Path.GetFileNameWithoutExtension(baseName);

            var exportedAny = false;

            for (var i = 0; i < ClumpsWrapper.Count; i++)
            {
                var clumpWrapper = (RwClumpNodeWrapper)ClumpsWrapper.Nodes[i];

                var name = ClumpsWrapper.Count == 1 ? baseName : $"{baseName}_{clumpWrapper.Text.Replace(' ', '_')}";
                var path = Path.Combine(folderPath, Path.ChangeExtension(name, "dae"));

                clumpWrapper.Export(path, SupportedFileType.AssimpModelFile);
                exportedAny = true;
            }
    ...
    public class RwTextureDictionaryNodeWrapper : RwNodeWrapperBase<RwTextureDictionaryNode>
    {
        ...
        /// <summary>
        /// Dumps every texture in this dictionary as a PNG directly into the given folder,
        /// mirroring the existing "Export All" context menu action.
        /// </summary>
        public override bool ExportToFolder(string folderPath)
        {
            if (Resource.TextureCount == 0)
                return false;
            ...
            foreach (var texture in Resource.Textures)
            {
                try
                {
                    var path = Path.Combine(folderPath, Path.ChangeExtension(texture.Name, "png"));
